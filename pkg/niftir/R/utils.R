# assume repeated measure
# subjects or random variable in rows
# conditions or repeated measure in cols
niftir.f.test <- function(x) {
    n <- nrow(x)
    k <- ncol(x)
    
    ssq <- function(vec, n=1) sum(n*(vec-mean(vec))^2)
    # sum of squares for condition
    ssC <- ssq(colMeans(x), n)
    # sum of squares for voxels
    ssV <- ssq(rowMeans(x), k)
...
niftir.friedman.test <- function(x) {
    k <- ncol(x)
    n <- nrow(x)
    
    if (k!=2)
        stop("can only accept x with 2 columns")
