niftir.colmean <- function(x, cols=1:ncol(x), rows=1:nrow(x), ztransform=FALSE, 
                            block.size=1000, verbose=T, na.rm=FALSE) 
{
    if (ztransform)
        call.fun <- "BigColZMeanMain"
    else
        call.fun <- "BigColMeanMain"
    
    ncols <- length(cols)
    blocks <- niftir.split.indices(1, ncols, by=block.size)
    
    if (verbose) {
        cat("Calculating column means\n")
        pb <- progressbar(blocks$n)
    }
    
    cms <- vector("numeric", ncols)
    for (i in 1:blocks$n) {
        if (verbose)
            update(pb, i)
    
        b.inds <- blocks$starts[i]:blocks$ends[i]
        cms[b.inds] <- .Call(call.fun, x@address, as.double(cols[b.inds]), as.double(rows), na.rm)
        
        if (is.filebacked(x))
            x <- free.memory(x)
    }
    
    if (verbose)
        close(pb)
    
    return(cms)
}

#' @nord
zarrar.deepcopy <- function(x, cols=NULL, rows=NULL, type = NULL, separated = NULL, backingfile = NULL, backingpath = NULL, descriptorfile = NULL, shared = TRUE) {
    if (!is.big.matrix(x))
        stop("input must be a big.matrix")
    if (!is.null(type))
        stop("unsupported to supply your own type")
    if (!is.null(separated))
        stop("unsupported to supply your own seperated format")
    if (is.null(cols))
        cols <- 1:ncol(x)
    if (is.null(rows))
        rows <- 1:nrow(x)
    
    bigmat <- big.matrix(length(rows), length(cols), type=typeof(x), separated=is.separated(x), shared=is.shared(x), backingfile=backingfile, backingpath=backingpath, descriptorfile=descriptorfile)
    
    .Call("BigDeepCopyMain", x@address, bigmat@address, as.double(rows), as.double(cols))
    
#    if (is.big.niftiXd(x)) {
#        # create new class
#        args <- list(class(x))
#        xslots <- lapply(slotNames(x), function(sn) slot(x, sn))
#        names(xslots) <- slotNames(x)
#        xslots$address <- bigmat@address
#        args <- append(args, xslots)
#        y <- do.call("new", args)
#    } else {
#        y <- bigmat
#    }
#    
    return(bigmat)
}

#' @nord
setGeneric("scale",
    function(x, center=TRUE, scale=TRUE, ...)
        standardGeneric('scale')
)

#' @nord
setMethod("scale",
    signature(x="big.matrix"),
    function(x, center=TRUE, scale=TRUE, to.copy=TRUE) {
        # Copy
        if (to.copy)
            y <- zarrar.deepcopy(x)
        else
            y <- x

        # Get mean and/or sd column values
        # and center and/or scale
        if (center) {
            xmean <- colmean(y, na.rm=TRUE)
            .Call("BigSubtractColsMain", y@address, as.double(xmean), TRUE)
        }
        if (scale) {
            xsd <- colsd(y, na.rm=TRUE)
            .Call("BigDivideColsMain", y@address, as.double(xsd), TRUE)
        }
        
        return(y)
    }
)

#' @nord
setMethod("scale",
    definition=function(x, center=TRUE, scale=TRUE)
        scale.default(x, center, scale)
)

#' @nord
setMethod('diag', 
    signature(x="big.matrix"),
    function(x) {
        len <- min(dim(x))
        .Call("GetDiagMain", x@address, as.double(len))
    }
)

setMethod('diag<-', 
    signature(x="big.matrix"),
    function(x, value) {
        if (length(value) == 1)
            values <- rep(value, min(dim(x)))
        else if (length(value) == min(dim(x)))
            values <- value
        else
            stop("replacement diagonal has wrong length")
        .Call("SetDiagMain", x@address, as.double(values))
        return(x)
    }
)

#' @nord
setGeneric("do.operator",
    function(x, y, operator, z, ...)
        standardGeneric('do.operator')
)

#' @nord
setMethod("do.operator",
    signature(x="big.matrix", y="big.matrix", operator="character", z="big.matrix"),
    function(x, y, operator, z, xCols=1:ncol(x), yCols=1:ncol(y), zCols=1:ncol(z), xRows=1:nrow(x), yRows=1:nrow(y), zRows=1:nrow(z)) {
        if (!all(length(xCols)==length(yCols)) && !all(length(xCols)==length(zCols)))
            stop("Length of column indices must be the same across matrices")
        if (!all(length(xRows)==length(yRows)) && !all(length(xRows)==length(zRows)))
            stop("Length of row indices must be the same across matrices")
        switch(operator,
            '+' = .Call("BigAddMatsMain", x@address, y@address, z@address, as.double(xCols), as.double(yCols), as.double(zCols), as.double(xRows), as.double(yRows), as.double(zRows)),
            '-' = .Call("BigSubtractMatsMain", x@address, y@address, z@address, as.double(xCols), as.double(yCols), as.double(zCols), as.double(xRows), as.double(yRows), as.double(zRows)),
            '*' = .Call("BigMultiplyMatsMain", x@address, y@address, z@address, as.double(xCols), as.double(yCols), as.double(zCols), as.double(xRows), as.double(yRows), as.double(zRows)),
            '/' = .Call("BigDivideMatsMain", x@address, y@address, z@address, as.double(xCols), as.double(yCols), as.double(zCols), as.double(xRows), as.double(yRows), as.double(zRows)),
            stop("operator must be +, -, *, or /")
        )
        return(NULL)
    }
)

#' @nord
# NOTE: THIS IS CURRENTLY SPECIFIC TO NEUROCOR/6D OBJECTS...BAD
setMethod("do.operator",
    signature(x="big.matrix", y="big.matrix", operator="character", z="missing"),
    function(x, y, operator, ...) {
        zlist <- niftir.big.matrix(nrow(x), ncol(x), ...)
        do.operator(x, y, operator, zlist$bm)
        if (is.big.niftiXd(x)) {
            # create new class
            args <- list(class(x))
            zslots <- lapply(slotNames(x), function(sn) slot(x, sn))
            names(zslots) <- slotNames(x)
            zslots$address <- zlist$bm@address
            zslots$backingfile <- zlist$bf
            zslots$descriptorfile <- zlist$df
            args <- append(args, zslots)
            z <- do.call("new", args)
            describe.ncor <- list(descriptorfile=args$descriptorfile, backingfile=args$backingfile, header=args@header, mask=args@mask, rowmask=args@mask)
            fname <- sprintf("%s.neurocor", rmext(args$backingfile))
            dput(describe.ncor, file=fname)
        } else {
            z <- zlist$bm
        }
        return(z)
    }
)

#' @nord
setGeneric("do.operatorinblocks",
    function(x, y, operator, z, blocksize=1000, ...)
        standardGeneric('do.operatorinblocks')
)

setMethod("do.operatorinblocks",
    signature(x="big.niftiXd", y="big.niftiXd", operator="character", z="character"),
    function(x, y, operator, z, xCols=1:ncol(x), xRows=1:nrow(x), ...) {
        
        ylist <- niftir.big.matrix(length(xRows), length(xCols), output=z)
        
        # New class
        colmask <- x@mask
        rowmask <- x@rowmask
        tmp <- vector("logical", sum(colmask))
        tmp[xCols] <- TRUE
        colmask[colmask] <- tmp
        tmp <- vector("logical", sum(rowmask))
        tmp[xRows] <- TRUE
        rowmask[rowmask] <- tmp
        ncor <- new("big.nifti6d", address=ylist$bm@address, header=x@header, mask=colmask, descriptorfile=ylist$df, backingfile=ylist$bf, rowmask=rowmask)
        
        # Save info
        describe.ncor <- list(descriptorfile=ylist$df, backingfile=ylist$bf, header=ncor@header, mask=ncor@mask, rowmask=ncor@rowmask)
        fname <- sprintf("%s.neurocor", rmext(describe.ncor$backingfile))
        dput(describe.ncor, file=fname)
        
        # Call
        do.operatorinblocks(x, y, operator, ncor, xCols=xCols, xRows=xRows, ...)
    }
)

#' @nord
setMethod("do.operatorinblocks",
    signature(x="big.niftiXd", y="big.niftiXd", operator="character", z="big.niftiXd"),
    function(x, y, operator, z, blocksize=2500, verbose=T, xCols=1:ncol(x), yCols=1:ncol(y), zCols=1:ncol(z), xRows=1:nrow(x), yRows=1:nrow(y), zRows=1:nrow(z)) {
        if (!all(length(xCols)==length(yCols)) && !all(length(xCols)==length(zCols)))
            stop("Length of column indices must be the same across matrices")
        if (!all(length(xRows)==length(yRows)) && !all(length(xRows)==length(zRows)))
            stop("Length of row indices must be the same across matrices")
        
        nc <- length(xCols)
        nr <- length(xRows)
        
        blocks <- niftir.split.indices(1, nc, by=blocksize)
        
        ofun <- function(j) {
            do.operator(x, y, operator, z, xCols=xCols[j], yCols=yCols[j], zCols=zCols[j], xRows=xRows, yRows=yRows, zRows=zRows)
        }
        
        if (verbose)
            pb <- progressbar(blocks$n)
        for (bi in 1:blocks$n) {
            if (verbose) {
                update(pb, bi)
                stime <- proc.time()[3]
            }
            
            if (getOption("neurocor.allow.parallel") && getDoParRegistered())
                foreach(j=blocks$starts[bi]:blocks$ends[bi]) %dopar% ofun(j)
            else
                ofun(blocks$starts[bi]:blocks$ends[bi])
            
            x <- free.memory(x)
            y <- free.memory(y)
            z <- free.memory(z)
            
            if (verbose) {
                etime <- proc.time()[3]
                totaltime <- ((etime-stime)*(blocks$n-bi))/60 # minutes
                cat("\nEstimated completion time:", totaltime, "mins\n")
            }
        }
        if (verbose)
            end(pb)
        
        return(z)
    }
)

# Calculate the sum across many big matrices
setGeneric("sum.bigmats",
    function(x, blocksize=1000, verbose=TRUE, ...)
        standardGeneric('sum.bigmats')
)

setMethod("sum.bigmats",
    signature(x="list"),
    function(x, blocksize=1000, colInds=NULL, rowInds=NULL, verbose=T, toSquare=F, backingpaths=NULL) {
        
        if (is.big.niftiXd(x[[1]]))
            backingpaths <- sapply(x, function(xx) dirname(xx@backingfile))
        if (!is.null(backingpaths) && length(x) != length(backingpaths))
            stop("list length between x and backingpaths input must be the same")
        
        nx <- length(x)
        # Check that lengths match
        if (!is.null(colInds) && length(x) != length(colInds))
            stop("list length between x and colInds input must be the same")
        if (!is.null(rowInds) && length(x) != length(rowInds))
            stop("list length between x and rowInds input must be the same")
        # Check that all objects the same
        out.type <- typeof(x[[1]])
        if (!all(out.type == sapply(x[-1], typeof)))
            stop("All input objects must have the same data type")
        # Check columns
        if (is.null(colInds)) {
            refCol <- ncol(x[[1]])
            colInds <- lapply(1:nx, function(i) {
                iCol <- ncol(x[[i]])
                if (iCol != refCol)
                    stop("All inputs must have the same column dimensions")
                iCol
            })
        } else {
            refCol <- length(colInds[[1]])
            lapply(1:nx, function(i) {
                if (length(colInds[[1]]) != refCol)
                    stop("All inputs must have the same number of column indices")
            })
        }
        # Check rows
        if (is.null(rowInds)) {
            refRow <- nrow(x[[1]])
            rowInds <- lapply(1:nx, function(i) {
                iRow <- nrow(x[[i]])
                if (iRow != refRow)
                    stop("All inputs must have the same row dimensions")
                iRow
            })
        } else {
            refRow <- length(rowInds[[1]])
            lapply(1:nx, function(i) {
                if (length(rowInds[[1]]) != refRow)
                    stop("All inputs must have the same number of row indices")
            })
        }
        
        # Create a new bigmat object
        outmat <- big.matrix(refRow, refCol, type=out.type, init=0)
        
        # Sum Function
        if (toSquare) {
            # Sum Squared Function
            sfun <- function(j) {
                # Copy over to memory
                tmp.mat <- zarrar.deepcopy(x[[xi]], cols=colInds[[xi]][j], rows=rowInds[[xi]])

                # Square
                .Call("BigPowMain", tmp.mat@address, PACKAGE="niftir")

                # Add Together
                do.operator(tmp.mat, outmat, "+", outmat, yCols=j, zCols=j)
            }
        } else {
            sfun <- function(j) {
                do.operator(x[[xi]], outmat, "+", outmat, xCols=colInds[[xi]][j], xRows=rowInds[[xi]], yCols=j,  zCols=j)
            }
        }
        
        # Loop through elements adding to output
        blocks <- niftir.split.indices(1, refCol, by=blocksize)
        #isFB <- sapply(x, function(xx) is.filebacked(xx))
        #anyFB <- any(isFB)
        if (verbose)
            pb <- progressbar(blocks$n)
        for (bi in 1:blocks$n) {
            if (verbose) {
                update(pb, bi)
                stime <- proc.time()[3]
            }
            
            for (xi in 1:nx) {
                # Break up block into sub-blocks
                if (getOption("neurocor.allow.parallel") && getDoParRegistered())
                    foreach(j=blocks$starts[bi]:blocks$ends[bi]) %dopar% sfun(j)
                else
                    sfun(blocks$starts[bi]:blocks$ends[bi])
                x[[xi]] <- free.memory(x[[xi]], backingpath=backingpaths[[xi]])
            }
            
            if (verbose) {
                etime <- proc.time()[3]
                totaltime <- ((etime-stime)*(blocks$n-bi))/60 # minutes
                cat("\nEstimated completion time:", totaltime, "mins\n")
            }
        }
        
        return(outmat)
    }
)

# Calculate the mean across many big matrices
setGeneric("mean.bigmats",
    function(x, ...)
        standardGeneric('mean.bigmats')
)

setMethod("mean.bigmats",
    signature(x="list"),
    function(x, ...) {
        outmat <- sum.bigmats(x, ...)
        .Call("BigDivideScalarMain", outmat@address, as.double(length(x)), TRUE, PACKAGE="niftir")
        return(outmat)
    }
)

# Calculate variance across many big matrices
setGeneric("var.bigmats",
    function(x, mx, ...)
        standardGeneric('var.bigmats')
)

setMethod("var.bigmats",
    signature(x="list", mx="big.matrix"),
    function(x, mx, blocksize=1000, colInds=NULL, rowInds=NULL, verbose=T, mx.backingpath=NULL) {
        
        # Check that lengths match
        if (!is.null(colInds) && length(x) != length(colInds))
            stop("list length between x and colInds input must be the same")
        if (!is.null(rowInds) && length(x) != length(rowInds))
            stop("list length between x and rowInds input must be the same")
        
        nx <- length(x)
        # Check that all objects the same
        out.type <- typeof(x[[1]])
        if (!all(out.type == sapply(x[-1], typeof)))
            stop("All input objects must have the same data type")
        # Check columns
        if (is.null(colInds)) {
            refCol <- ncol(mx)
            colInds <- lapply(1:nx, function(i) {
                iCol <- ncol(x[[i]])
                if (iCol != refCol)
                    stop("All inputs must have the same column dimensions")
                iCol
            })
        } else {
            refCol <- ncol(mx)
            lapply(1:nx, function(i) {
                if (length(colInds[[1]]) != refCol)
                    stop("All inputs must have the same number of column indices")
            })
        }
        # Check rows
        if (is.null(rowInds)) {
            refRow <- nrow(mx)
            rowInds <- lapply(1:nx, function(i) {
                iRow <- nrow(x[[i]])
                if (iRow != refRow)
                    stop("All inputs must have the same row dimensions")
                iRow
            })
        } else {
            refRow <- nrow(mx)
            lapply(1:nx, function(i) {
                if (length(rowInds[[1]]) != refRow)
                    stop("All inputs must have the same number of row indices")
            })
        }
        
        # Create a new bigmat object
        outmat <- big.matrix(refRow, refCol, type=out.type, init=0)
        
        # Var Function
        sfun <- function(j) {
            # Create Temporary Big Matrix
            tmp.mat <- big.matrix(refRow, length(j), type=out.type, init=0)
            
            # De-mean [x-mean(x)]
            do.operator(x[[xi]], mx, "-", tmp.mat, xCols=colInds[[xi]][j], xRows=rowInds[[xi]], yCols=j)
            
            # Square [(x-mean(x))^2]
            .Call("BigPowMain", tmp.mat@address, PACKAGE="niftir")
            
            # Add Together
            do.operator(tmp.mat, outmat, "+", outmat, yCols=j, zCols=j)
            
            rm(tmp.mat)
            gc()
        }
        
        # Loop through elements adding to output
        blocks <- niftir.split.indices(1, refCol, by=blocksize)
        if (verbose)
            pb <- progressbar(blocks$n)
        for (bi in 1:blocks$n) {
            if (verbose) {
                update(pb, bi)
                stime <- proc.time()[3]
            }
            
            for (xi in 1:nx) {
                # Break up block into sub-blocks
                if (getOption("neurocor.allow.parallel") && getDoParRegistered())
                    foreach(j=blocks$starts[bi]:blocks$ends[bi]) %dopar% sfun(j)
                else
                    sfun(blocks$starts[bi]:blocks$ends[bi])
                x[[xi]] <- free.memory(x[[xi]])
            }
            mx <- free.memory(mx, mx.backingpath)
                        
            if (verbose) {
                etime <- proc.time()[3]
                totaltime <- ((etime-stime)*(blocks$n-bi))/60 # minutes
                cat("\nEstimated completion time:", totaltime, "mins\n")
            }
        }
        
        # Divide by df
        if (verbose)
            cat("Almost done...\n")
        .Call("BigDivideScalarMain", outmat@address, as.double(length(x)-1), TRUE, PACKAGE="niftir")
        
        return(outmat)
    }
)


# Calculate t-tests
# Calculate one-sample t-test across many big matrices
setGeneric("onesample.tt.bigmats",
    function(mmat, vmat, n, ...)
        standardGeneric('onesample.tt.bigmats')
)

setMethod("onesample.tt.bigmats",
    signature(mmat="big.matrix", vmat="big.matrix", n="integer"),
    function(mmat, vmat, n, blocksize=1000, verbose=T, mmat.backingpath=NULL, vmat.backingpath=NULL) {
        
        # Check that all objects the same
        if (typeof(mmat) != typeof(vmat))
            stop("All input objects must have the same datatype")
        # Check dimensions
        if (!all(dim(mmat)==dim(vmat)))
            stop("All input objects must have the same dimensions")
        
        # Create a new bigmat object
        nr <- nrow(mmat)
        nc <- ncol(mmat)
        ot <- typeof(mmat)
        outmat <- big.matrix(nr, nc, type=ot, init=0)
        
        # Var Function
        sfun <- function(j) {
            # Copy over variance to memory
            semat <- zarrar.deepcopy(vmat, cols=j)
            
            # Standard-Deviation [sqrt(var)]
            .Call("BigSqrtMain", semat@address, PACKAGE="niftir")
            
            # Divide by sqrt(N) to get standard-error
            .Call("BigDivideScalarMain", semat@address, as.double(sqrt(n)), TRUE, PACKAGE="niftir")
            
            # Divide mean by se
            do.operator(mmat, semat, "/", outmat, xCols=j, zCols=j)
                        
            rm(semat)
            gc()
        }
        
        # Loop through elements adding to output
        blocks <- niftir.split.indices(1, nc, by=blocksize)
        if (verbose)
            pb <- progressbar(blocks$n)
        for (bi in 1:blocks$n) {
            if (verbose) {
                update(pb, bi)
                stime <- proc.time()[3]
            }
            
            # Break up block into sub-blocks
            if (getOption("neurocor.allow.parallel") && getDoParRegistered())
                foreach(j=blocks$starts[bi]:blocks$ends[bi]) %dopar% sfun(j)
            else
                sfun(blocks$starts[bi]:blocks$ends[bi])
            
            mmat <- free.memory(mmat, mmat.backingpath)
            vmat <- free.memory(vmat, vmat.backingpath)
            
            if (verbose) {
                etime <- proc.time()[3]
                totaltime <- ((etime-stime)*(blocks$n-bi))/60 # minutes
                cat("\nEstimated completion time:", totaltime, "mins\n")
            }
        }
        
        return(outmat)
    }
)

# Calculate paired t-test across many big matrices
setGeneric("paired.tt.bigmats",
    function(mmat1, mmat2, vmat1, vmat2, n, ...)
        standardGeneric('paired.tt.bigmats')
)

setMethod("paired.tt.bigmats",
    signature(mmat1="big.matrix", mmat2="big.matrix", vmat1="big.matrix", vmat2="big.matrix", n="integer"),
    function(mmat1, mmat2, vmat1, vmat2, n, blocksize=1000, verbose=T, mmat1.backingpath=NULL, mmat2.backingpath=NULL, vmat1.backingpath=NULL, vmat2.backingpath=NULL) {
        
        # Check that all objects the same
        if ((typeof(mmat1) != typeof(mmat2)) || (typeof(vmat1) != typeof(vmat2)) || (typeof(mmat1) != typeof(vmat1)))
            stop("All input objects must have the same datatype")
        # Check dimensions
        if (!all(dim(mmat1) == dim(mmat2)) || !all(dim(vmat1) == dim(vmat2)) || !all(dim(mmat1) == dim(vmat1)))
            stop("All input objects must have the same dimensions")
        
        # Create a new bigmat object
        nr <- nrow(mmat1)
        nc <- ncol(mmat1)
        ot <- typeof(mmat1)
        outmat <- big.matrix(nr, nc, type=ot, init=0)
        
        # Var Function
        sfun <- function(j) {
            # Copy over variances to memory
            sdmat1 <- zarrar.deepcopy(vmat1, cols=j)
            sdmat2 <- zarrar.deepcopy(vmat2, cols=j)
            
            # Get mean difference
            mmat <- big.matrix(nr, length(j), type=ot, init=0)
            do.operator(mmat1, mmat2, "-", mmat, xCols=j, yCols=j)
            
            # Get standard-deviations
            .Call("BigSqrtMain", sdmat1@address, PACKAGE="niftir")
            .Call("BigSqrtMain", sdmat2@address, PACKAGE="niftir")
            
            # Get sd difference
            semat <- sdmat1
            do.operator(sdmat1, sdmat2, "-", semat)
            
            # Get se
            .Call("BigDivideScalarMain", semat@address, as.double(sqrt(n)), TRUE, PACKAGE="niftir")
            
            # Divide mean by se
            do.operator(mmat, semat, "/", outmat, zCols=j)
                   
            rm(mmat)
            rm(semat)
            gc()
        }
        
        # Loop through elements adding to output
        blocks <- niftir.split.indices(1, nc, by=blocksize)
        if (verbose)
            pb <- progressbar(blocks$n)
        for (bi in 1:blocks$n) {
            if (verbose) {
                update(pb, bi)
                stime <- proc.time()[3]
            }
            
            # Break up block into sub-blocks
            if (getOption("neurocor.allow.parallel") && getDoParRegistered())
                foreach(j=blocks$starts[bi]:blocks$ends[bi]) %dopar% sfun(j)
            else
                sfun(blocks$starts[bi]:blocks$ends[bi])
            
            mmat1 <- free.memory(mmat1, mmat1.backingpath)
            vmat1 <- free.memory(vmat1, vmat1.backingpath)
            mmat2 <- free.memory(mmat2, mmat2.backingpath)
            vmat2 <- free.memory(vmat2, vmat2.backingpath)
            
            if (verbose) {
                etime <- proc.time()[3]
                totaltime <- ((etime-stime)*(blocks$n-bi))/60 # minutes
                cat("\nEstimated completion time:", totaltime, "mins\n")
            }
        }
        
        return(outmat)
    }
)

# Calculate unpaired two-sample t-test across many big matrices
setGeneric("unpaired.tt.bigmats",
    function(mmat1, mmat2, vmat1, vmat2, n1, n2, ...)
        standardGeneric('unpaired.tt.bigmats')
)

setMethod("unpaired.tt.bigmats",
    signature(mmat1="big.matrix", mmat2="big.matrix", vmat1="big.matrix", vmat2="big.matrix", n1="integer", n2="integer"),
    function(mmat1, mmat2, vmat1, vmat2, n1, n2, blocksize=1000, verbose=T, mmat1.backingpath=NULL, mmat2.backingpath=NULL, vmat1.backingpath=NULL, vmat2.backingpath=NULL) {
        
        # Check that all objects the same
        if ((typeof(mmat1) != typeof(mmat2)) || (typeof(vmat1) != typeof(vmat2)) || (typeof(mmat1) != typeof(vmat1)))
            stop("All input objects must have the same datatype")
        # Check dimensions
        if (!all(dim(mmat1) == dim(mmat2)) || !all(dim(vmat1) == dim(vmat2)) || !all(dim(mmat1) == dim(vmat1)))
            stop("All input objects must have the same dimensions")
        
        # Create a new bigmat object
        nr <- nrow(mmat1)
        nc <- ncol(mmat1)
        ot <- typeof(mmat1)
        outmat <- big.matrix(nr, nc, type=ot, init=0)
        
        # Var Function
        sfun <- function(j) {
            # Copy over variances to memory
            tmp.vmat1 <- zarrar.deepcopy(vmat1, cols=j)
            tmp.vmat2 <- zarrar.deepcopy(vmat2, cols=j)
            
            # Get mean difference
            mmat <- big.matrix(nr, length(j), type=ot, init=0)
            do.operator(mmat1, mmat2, "-", mmat, xCols=j, yCols=j)
            
            # Divide variance by n
            .Call("BigDivideScalarMain", tmp.vmat1@address, as.double(n1), TRUE, PACKAGE="niftir")
            .Call("BigDivideScalarMain", tmp.vmat2@address, as.double(n2), TRUE, PACKAGE="niftir")
            
            # Add two variances together
            semat <- tmp.vmat1
            do.operator(tmp.vmat1, tmp.vmat2, "+", semat)
            
            # Take square-root
            .Call("BigSqrtMain", semat@address, PACKAGE="niftir")
            
            # Divide mean by se
            do.operator(mmat, semat, "/", outmat, zCols=j)
                   
            rm(mmat)
            rm(semat)
            gc()
        }
        
        # Loop through elements adding to output
        blocks <- niftir.split.indices(1, nc, by=blocksize)
        if (verbose)
            pb <- progressbar(blocks$n)
        for (bi in 1:blocks$n) {
            if (verbose) {
                update(pb, bi)
                stime <- proc.time()[3]
            }
            
            # Break up block into sub-blocks
            if (getOption("neurocor.allow.parallel") && getDoParRegistered())
                foreach(j=blocks$starts[bi]:blocks$ends[bi]) %dopar% sfun(j)
            else
                sfun(blocks$starts[bi]:blocks$ends[bi])
            
            mmat1 <- free.memory(mmat1, mmat1.backingpath)
            vmat1 <- free.memory(vmat1, vmat1.backingpath)
            mmat2 <- free.memory(mmat2, mmat2.backingpath)
            vmat2 <- free.memory(vmat2, vmat2.backingpath)
            
            if (verbose) {
                etime <- proc.time()[3]
                totaltime <- ((etime-stime)*(blocks$n-bi))/60 # minutes
                cat("\nEstimated completion time:", totaltime, "mins\n")
            }
        }
        
        return(outmat)
    }
)
