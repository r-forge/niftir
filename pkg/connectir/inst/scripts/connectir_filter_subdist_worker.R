...
option_list <- list(
    ...
    make_option("--nogower", type="store_true", default=FALSE, help="Don't run and save gower centered distance matrices"), 
    ...
    make_option("--memlimit", type="integer", default=4, dest="memlimit", help="If blocksize is set to auto (--blocksize=0), then will set the blocksize to use a maximum of RAM specified by this option  [default: %default]", metavar="number"),
...
tryCatch({
  ...
  s.path$bfile <- sprintf("%s.bin", basename(outprefix))
  s.path$dfile <- sprintf("%s.desc", basename(outprefix))
  ...
  g.path$bfile <- sprintf("%s_gower.bin", basename(outprefix))
  g.path$dfile <- sprintf("%s_gower.desc", basename(outprefix))
  ...
  tmp <- attach.big.matrix(s.path$dfile)
  nsubs <- sqrt(nrow(tmp))
  nvoxs <- ncol(tmp)
